...
@section('content')
    ...
    <div class="container page_create_post">
        ...
        <form id="form" action="/analyst/countrycatalog/addregion" method="post" >
            ...
            <div class="row">
	            <p class="d-flex justify-content-start">
		            <span class="name">Отчет: </span><span class="text">{{ $reporttitle }} <!--№ {{ $countrycatalog->number }}--> за период от {{ date("d.m.Y",$countrycatalog->start_date)}} до {{ date("d.m.Y",$countrycatalog->end_date)}}</span>
		        </p>
	        </div>
            ...
            <div class="row box_save_article mt30">
              ...
              <button onclick="jQuery('#form').attr('action','/analyst/countrycatalog/addregion/1'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и отправить на утверждение</button>
            </div>
        </form>
    </div>
@endsection