...
@section('content')
    ...
    <div class="container page_create_post">
        <form id="form" action="/analyst/various/upd" method="post" enctype="multipart/form-data">
            <div class="row justify-content-center posr">
                <h3>Редактирование материала</h3>
                
                <span class="pos_tr_article_out status st-{{ $variousarticle->published }}">
                	@if($variousarticle->published == 0)
                		<span class="status st_inherit">Статус:</span> Не утвержден
                	@elseif($variousarticle->published == 1)
                ...
                <div class="col-md-12 box_info">
                    <p class="d-flex justify-content-start">
                        <span class="name">Отчет: </span>
                        <span class="text">{{ $variousreport->number }}
                    ...
                    {{--<p class="d-flex justify-content-start">--}}
                        {{--@if(isset($variousarticle->subcategory))--}}
                            ...
                            {{--<span class="italic_14">{{ $variousarticle->subcategory->category->title }}</span>--}}
                        {{--@else--}}
                            {{--<span class="italic_14">{{ $variousarticle->category->title }}</span>--}}
                        {{--@endif--}}
                    {{--</p>--}}
                    <input type="text" hidden name="year" value="<?= $y?>">
                    <input type="text" hidden name="month" value="<?= $m?>">
                    <input type="text" hidden name="variousarticle" value="{{ $variousarticle->id }}">
            ...
            <div class="row justify-content-start mb-3">
                <div class="coll_left">
                    <span class="name">Заголовок материала: </span>
                </div>
                <div class="coll_right">
                    <input type="text" class="title_post" name="title" placeholder="Введите заголовок" value="{{$variousarticle->title}}"/>
                </div>
            </div>
            <div class="row justify-content-start mb_3">
                ...
                <div class="coll_right">
                    <textarea name="editor1">{{ $variousarticle->body }}</textarea>
                </div>
            </div>
            <div class="row justify-content-start mb_3">
                <div class="coll_left">
                ...
                <div class="coll_right d-flex justify-content-between box_add_gallery">
					        <?php $count_images = 0; ?>
                    @if(isset($variousarticle->images))
                        @foreach($variousarticle->images as $image)
                        	<?php $count_images++; ?>
                            ...
                            <label class="file_label" for="input_<?php echo $count_images; ?>">
                                ...
                                <!-- <b class="del_img_gal">Нажмите, чтобы удалить изображение!</b> -->
                                </label>
            ...
            <div class="row box_save_article mt30">           	
              ...
              @if($variousarticle->published == 0)
                ...
                <button onclick="jQuery('#form').attr('action','/analyst/various/upd/1'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и отправить на утверждение</button>
              @elseif($variousarticle->published == 1)
                <a href="/analyst/various/add2/{{ $variousreport->id }}" class="button butt_back">Вернуться к отчету</a>
                    <button class="button_save butt butt_def">Сохранить</button>
                    <button onclick="jQuery('#form').attr('action','/analyst/various/upd/2'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и утвердить</button>
              @else
                <a href="/analyst/various/add2/{{$variousreport->id}}" class="button butt_back">Вернуться к отчету</a>
                <button class="button_save butt butt_def">Сохранить</button>
              @endif              
            ...
            <div class="row name_report name_report_step3 d-flex justify-content-center">
            	<div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_left">
		            ...
		            <span class="select_wrap calendar_wrap" style="display: none;">
			        	...
			        	<input type="hidden" value="{{ $variousreport->start_date }}" name="start_period">
		        	</span>
		        	</div>
		        	<div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_right">
		                ...
		                <span class="select_wrap calendar_wrap" style="display: none;">
				        	...
				        	<input type="hidden" value="{{ $variousreport->end_date }}" name="end_period">
				        </span>
			       </div>
            </div>
            </br>
    ...
    <script type="text/javascript" charset="utf-8">
        jQuery(document).ready(function () {
            ...
            jQuery('.calendar_start_3').datepicker({
                ...
                locale: 'ru-ru',
                minDate: '<?php echo date("d.m.Y",$variousreport->start_date); ?>',
                maxDate: '<?php echo date("d.m.Y",$variousreport->end_date); ?>',
                value: '<?php echo date("d.m.Y",$variousarticle->start_period); ?>',
                format: 'dd.mm.yyyy',
            });
            ...
            jQuery('.calendar_end_3').datepicker({
                ...
                minDate: '<?php echo date("d.m.Y",$variousreport->start_date); ?>',
                maxDate: '<?php echo date("d.m.Y",$variousreport->end_date); ?>',
                value: '<?php echo date("d.m.Y",$variousarticle->end_period); ?>',
                format: 'dd.mm.yyyy',
