...
@section('content')
    <div class="container add_form_step1">
        ...
        <form action="/analyst/various/updreport/{{$variousreport->id}}" method="post">
            @csrf
            <input type="text" hidden name="year" value="<?= $y?>">
            <input type="text" hidden name="month" value="<?= $m?>">
            <div class="row name_report">
                <span>Отчет "{{$variousreport->number}}" за период от</span>
                ...
                <span>
		        	<input name="number" value="{{$variousreport->number}}" placeholder="{{$variousreport->number}}" style="width: 550px !important;"/>
		        </span>
            </div>
            <div class="row justify-content-center">
