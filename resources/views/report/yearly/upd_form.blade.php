...
@section('content')
    ...
    <div class="container page_create_post">
        <form id="form" action="/analyst/yearly/upd" method="post" enctype="multipart/form-data">
            <div class="row justify-content-center posr">
                <h3>Редактирование материала для отчета</h3>
                
                <span class="pos_tr_article_out status st-{{$yearlyarticle->published}}">
                	@if($yearlyarticle->published == 0)
                		<span class="status st_inherit">Статус:</span> Не утвержден
                	@elseif($yearlyarticle->published == 1)
                ...
                <div class="col-md-12 box_info">
                    <p class="d-flex justify-content-start">
                        ...
                        <span class="text">"{{ $reporttitle }}" за
                            {{ date("Y",$yearlyreport->start_date)}} год
                        </span>
                    </p>
                    <p class="d-flex justify-content-start">
                		@if(isset( $yearlyarticle->subcategory ))
                            <span class="name">Подраздел: </span>
                        
		                	<span class="text">
		                		{{ $yearlyarticle->subcategory->category->title }}
		                	</span>
	                	@endif
		          	</p>
		          	<p class="d-flex justify-content-start">   
		               	@if(isset( $yearlyarticle->category->title ))
	                        <span class="name">Раздел: </span>
	                        
	                       	<span class="text">{{ $yearlyarticle->category->title }}</span>
	                    @endif
                    </p>
                    <input type="text" hidden name="year" value="<?= $y?>">
                    <input type="text" hidden name="month" value="<?= $m?>">
                    <input type="text" hidden name="yearlyarticle" value="{{ $yearlyarticle->id }}">
                    <input type="text" hidden name="yearlyreport_id" value="{{ $yearlyreport->id }}">
                </div>
            ...
            <div class="row justify-content-start mb-3">
                ...
                <div class="coll_right">
                    <input type="text" class="title_post" name="title" placeholder="Введите заголовок" value="{{$yearlyarticle->title}}"/>
                </div>
            </div>
            <div class="row justify-content-start mb_3">
                ...
                <div class="coll_right">
                    <textarea name="editor1">{{ $yearlyarticle->body }}</textarea>
                </div>
            </div>
            <div class="row justify-content-start mb_3">
                <div class="coll_left">
            ...
            <div class="row box_save_article mt30">
              ...
              @if($yearlyarticle->published == 0)
               ...
               <button onclick="jQuery('#form').attr('action','/analyst/yearly/upd/1'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и отправить на утверждение</button>
              @elseif($yearlyarticle->published == 1)
                <a href="/analyst/yearly/add2/{{ $yearlyreport->id }}" class="button butt_back">Вернуться к отчету</a>
                    <button class="button_save butt butt_def">Сохранить</button>
                    <button onclick="jQuery('#form').attr('action','/analyst/yearly/upd/2'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и утвердить</button>
              @else
                <a href="/analyst/yearly/add2/{{$yearlyreport->id}}" class="button butt_back">Вернуться к отчету</a>
                <button class="button_save butt butt_def">Сохранить</button>
              @endif
            ...
            <div class="row name_report name_report_step3 d-flex justify-content-center">
            	<div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_left">
		            ...
		            <span class="select_wrap calendar_wrap" style="display: none;">
			        	...
			        	<input type="hidden" value="{{ $yearlyarticle->start_period }}" name="start_period">
		        	</span>
		        	</div>
		        	<div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_right">
		                ...
		                <span class="select_wrap calendar_wrap" style="display: none;">
				        	...
				        	<input type="hidden" value="{{ $yearlyarticle->end_period }}" name="end_period">
				        </span>
			       </div>
            </div>
            </br>
       ...
       @if($yearlyarticle->published == 0)
            <form name="form_1" onsubmit="addApprove(this,'{{$yearlyarticle->title}}');return false;" action="/analyst/yearly/article_for_approval/{{ $yearlyarticle->id }}" method="post">
                {{ method_field('put') }}
                @csrf
            </form>
        @elseif($yearlyarticle->published == 1)
            <form name="form_2" onsubmit="approve(this,'{{$yearlyarticle->title}}');return false;" action="/analyst/yearly/article_publish/{{ $yearlyarticle->id }}" method="post">
                {{ method_field('put') }}
                @csrf
            </form>
        @endif
    ...
    <script type="text/javascript" charset="utf-8">
        jQuery(document).ready(function () {
            ...
            jQuery('.calendar_start_3').datepicker({
                ...
                value: '<?php echo date("d.m.Y",$yearlyarticle->start_period); ?>',
                format: 'dd.mm.yyyy',
            });
            ...
            jQuery('.calendar_end_3').datepicker({
                ...
                minDate: '<?php echo date("d.m.Y",$yearlyreport->start_date); ?>',
                maxDate: '<?php echo date("d.m.Y",$yearlyreport->end_date); ?>',
                value: '<?php echo date("d.m.Y",$yearlyarticle->end_period); ?>',
                format: 'dd.mm.yyyy',
