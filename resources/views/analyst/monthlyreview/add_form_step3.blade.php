...
@section('content')
    <div class="container page_create_post">
        <form id="form" action="/analyst/monthly/add3" method="post" enctype="multipart/form-data">
            <div class="row justify-content-center">
                <h3>Создание материала для отчета</h3>
                @csrf
                <div class="col-md-12 box_info">
                    <p class="d-flex justify-content-start">
                        <span class="name">Название: </span><span class="text">Некоторые аспекты развития вооружений, военной и специальной техники за рубежом № {{ $monthlyreport->number }} за период от {{date("d",$monthlyreport->start_date)}} {{mnames(date("m",$monthlyreport->start_date))}} {{date("Y",$monthlyreport->start_date)}} по {{date("d",$monthlyreport->end_date)}} {{mnames(date("m",$monthlyreport->end_date))}} {{date("Y",$monthlyreport->end_date)}}</span>
                    </p>
                    <p class="d-flex justify-content-start">
                        <span class="name">Раздел: </span><span class="text">{{ $category->title }}</span></p>
                    @if(isset($subcategory))
                    <p class="d-flex justify-content-start">
                        <span class="name">Подраздел: </span><span class="text">{{ $subcategory->title }}</span></p>
                        <input type="text" hidden name="subcategory" value="{{ $subcategory->id }}">
					@else
						<input type="text" hidden name="subcategory" value="">
                    @endif
                    <input type="text" hidden name="category" value="{{ $category->id }}">
                    <input type="text" hidden name="monthlyreport" value="{{ $monthlyreport->id }}">
            ...
            <div class="row box_save_article mt30">
                ...
                <button onclick="jQuery('#form').attr('action','/analyst/monthly/add3/1'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и отправить на утверждение</button>
            </div>
            <div class="row justify-content-center">
                <h3>Добавление поисковых меток</h3>
            </div>
            <div class="row name_report name_report_step3 d-flex justify-content-center">
	            ...
	            <div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_left">
		            ...
		            <span class="select_wrap calendar_wrap" style="display: none;">
			        	...
			        	<input type="hidden" value="<?php echo $monthlyreport->start_date; ?>" name="start_period">
		        	</span>
		        </div>
		        <div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_right">
		                ...
		                <span class="select_wrap calendar_wrap" style="display: none;">
				        	...
				        	<input type="hidden" value="<?php echo $monthlyreport->end_date; ?>" name="end_period">
				        </span>
			    </div>	
