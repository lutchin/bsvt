...
@section('content')
    <div class="container page_create_post posr">
        <form id="form" action="/analyst/monthly/upd" method="post" enctype="multipart/form-data">
            <div class="row justify-content-center">
                <h3>Создание материала для отчета</h3>
                
                <span class="pos_tr_article_out status st-{{$monthlyarticle->published}}">
                	@if($monthlyarticle->published == 0)
                		<span class="status st_inherit">Статус:</span> Не утвержден
                	@elseif($monthlyarticle->published == 1)
                ...
                <div class="col-md-12 box_info">
                    <p class="d-flex justify-content-start">
                        <span class="name">Название: </span><span class="text">Бюллетень военно-политической и военно-технической информации за период от {{ date("d.m.Y",$monthlyarticle->monthlyreport->start_date)}} до {{ date("d.m.Y",$monthlyarticle->monthlyreport->end_date)}}</span>
                    </p>
                    <p class="d-flex justify-content-start">
                        <span class="name">Раздел: </span><span class="text">{{ $monthlyarticle->category->title }}</span>
                    </p>
                    <input type="text" hidden name="year" value="<?= $y?>">
                    <input type="text" hidden name="month" value="<?= $m?>">
                    <input type="text" hidden name="monthlyarticle" value="{{ $monthlyarticle->id }}">
                </div>
            ...
            <div class="row justify-content-start mb-3">
                ...
                <div class="coll_right">
                    <input type="text" class="title_post" name="title" placeholder="Введите заголовок" value="{{$monthlyarticle->title}}"/>
                </div>
            </div>
            <div class="row justify-content-start mb_3">
                ...
                <div class="coll_right">
                    <textarea name="editor1">{{ $monthlyarticle->body }}</textarea>
                </div>
            </div>
            <div class="row justify-content-start mb_3">
                <div class="coll_left">
                ...
                <div class="coll_right d-flex justify-content-between box_add_gallery">
					        <?php $count_images = 0; ?>
                    @if(isset($monthlyarticle->monthlyimages))
                        @foreach($monthlyarticle->monthlyimages as $image)
                        	<?php $count_images++; ?>
            ...
            <div class="row box_save_article mt30">
              ...
              <button onclick="jQuery('#form').attr('action','/analyst/monthly/upd/1'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и отправить на утверждение</button>
          @elseif($monthlyarticle->published == 1)
            <a href="/analyst/monthly/add2/{{$monthlyarticle->monthlyreport->id}}" class="button butt_back">Вернуться к отчету</a>
                <button class="button_save butt butt_def">Сохранить</button>
                <button onclick="jQuery('#form').attr('action','/analyst/monthly/upd/2'); jQuery('#form').submit(); return false;" class="button_save butt butt_def">Сохранить и утвердить</button>
          @else
            <a href="/analyst/monthly/add2/{{$monthlyarticle->monthlyreport->id}}" class="button butt_back">Вернуться к отчету</a>
            <button class="button_save butt butt_def">Сохранить</button>
          @endif
            ...
            <div class="row name_report name_report_step3 d-flex justify-content-center">
            	<div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_left">
		            ...
		            <span class="select_wrap calendar_wrap" style="display: none;">
			        	...
			        	<input type="hidden" value="<?php echo $monthlyarticle->monthlyreport->start_date; ?>" name="start_period">
		        	</span>
		        	</div>
		        	<div class="col-md-4 float_left d-flex  flex-column justify-content-center align-items-start box_right">
		                ...
		                <span class="select_wrap calendar_wrap" style="display: none;">
				        	...
				        	<input type="hidden" value="<?php echo $monthlyarticle->monthlyreport->end_date; ?>" name="end_period">
				        </span>
			       </div>
            </div>
            </br>
        ...
        @if($monthlyarticle->published == 0)
            <form name="form_1" onsubmit="addApprove(this,'{{$monthlyarticle->title}}');return false;" action="/analyst/monthly/article_for_approval/{{ $monthlyarticle->id }}" method="post">
                {{ method_field('put') }}
                @csrf
            </form>
        @elseif($monthlyarticle->published == 1)
            <form name="form_2" onsubmit="approve(this,'{{$monthlyarticle->title}}');return false;" action="/analyst/monthly/article_publish/{{ $monthlyarticle->id }}" method="post">
                {{ method_field('put') }}
                @csrf
            </form>
        @endif
    ...
    <script type="text/javascript" charset="utf-8">
        jQuery(document).ready(function () {
            jQuery('.calendar_start_3').datepicker({
                ...
                locale: 'ru-ru',
                minDate: '<?php echo date("d.m.Y",$monthlyarticle->monthlyreport->start_date); ?>',
                maxDate: '<?php echo date("d.m.Y",$monthlyarticle->monthlyreport->end_date); ?>',
                value: '<?php echo date("d.m.Y",$monthlyarticle->start_period); ?>',
                format: 'dd.mm.yyyy',
            });
            ...
            jQuery('.calendar_end_3').datepicker({
                ...
                minDate: '<?php echo date("d.m.Y",$monthlyarticle->monthlyreport->start_date); ?>',
                maxDate: '<?php echo date("d.m.Y",$monthlyarticle->monthlyreport->end_date); ?>',
                value: '<?php echo date("d.m.Y",$monthlyarticle->end_period); ?>',
                format: 'dd.mm.yyyy',
